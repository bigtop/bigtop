...
trait Iso8601Format extends Format[String,DateTime,String] {
  ...
  val millisFormat = DateTimeFormat.forPattern(millisFormatString).withZone(DateTimeZone.UTC)
  val secondsFormat = DateTimeFormat.forPattern(secondsFormatString).withZone(DateTimeZone.UTC)

  /** Quick ISO8601-compatible timestamp-to-string conversion. */
  def write(time: DateTime): String =
    millisFormat.print(time.withZone(DateTimeZone.UTC))
