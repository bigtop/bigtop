...
class UuidSpec extends Specification {
  ...
  "Uuid.create()" should {
    "create a UUID that can be read by Uuid.parse" in {
      ...
      Uuid.parse(uuid.toString) must beSuccess(beEqualTo(uuid))
    }
  }

  "Uuid.parse" should {
    "succeed given valid string" in {
      Uuid.parse("aa6622d0-edc7-11e0-9d62-005056c00008").isSuccess mustEqual true
      Uuid.parse("AA6622D0-EDC7-11E0-9D62-005056C00008").isSuccess mustEqual true
    }

    "fail given invalid string" in {
      Uuid.parse("bad uuid") must beFailure()
    }
  }
}
